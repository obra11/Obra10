import { Controller, Post, UseInterceptors, UploadedFile, Param, BadRequestException, ForbiddenException, UseGuards, Req, ParseFilePipe, MaxFileSizeValidator, FileTypeValidator } from '@nestjs/common';
...
const ALLOWED_DOC_TYPES = /^(image\/(jpeg|jpg|png|gif|webp)|application\/(pdf|vnd\.openxmlformats-officedocument\.(spreadsheetml\.sheet|wordprocessingml\.document))|video\/mp4)$/;
...
export class UploadController {
  ...
  @UseInterceptors(FileInterceptor('file', {
    ...
    fileFilter: (_req, file, cb) => {
      if (!ALLOWED_IMAGE_TYPES.test(file.mimetype)) {
        return cb(new BadRequestException('Tipo de arquivo não permitido. Envie apenas imagens (jpg, png, gif, webp).'), false);
      }
      cb(null, true);
    },
  }))
  async uploadLogo(
    ...
    if (req.user.empresaId !== id && req.user.perfilGlobal !== 'SUPER_ADMIN') {
      throw new ForbiddenException('Sem permissão para alterar o logo desta empresa.');
    }
  ...
  @UseInterceptors(FileInterceptor('file', {
    ...
    fileFilter: (_req, file, cb) => {
      if (!ALLOWED_IMAGE_TYPES.test(file.mimetype)) {
        return cb(new BadRequestException('Tipo de arquivo não permitido. Envie apenas imagens.'), false);
      }
      cb(null, true);
    },
  }))
  async uploadUsuarioFoto(
    ...
    const isGestor = req.user.perfilGlobal === 'GESTOR' || req.user.perfilGlobal === 'SUPER_ADMIN';
    if (!isOwner && !isGestor) {
      throw new ForbiddenException('Sem permissão para alterar a foto deste usuário.');
    }
    if (!file) throw new BadRequestException('Nenhum arquivo enviado');
    const url = `/uploads/${file.filename}`;
  ...
  async uploadObraImage(
    ...
    const obra = await this.prisma.obra.findUnique({ where: { id }, select: { empresaId: true } });
  ...
  @UseInterceptors(FileInterceptor('file', {
    ...
    fileFilter: (_req, file, cb) => {
      if (!ALLOWED_DOC_TYPES.test(file.mimetype)) {
        return cb(new BadRequestException('Tipo de arquivo não permitido. Aceitos: jpg, png, pdf, xlsx, docx, mp4.'), false);
      }
      cb(null, true);
    },
  }))
