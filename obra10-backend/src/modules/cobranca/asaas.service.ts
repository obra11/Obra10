...
export class AsaasService {
  ...
  async criarClienteAsaas(empresa: {
    cpfCnpj: string; razaoSocial?: string; nomeCompleto?: string;
  ...
  async gerarCobrancaPix(dto: {
    ...
    descricao?: string;
  }): Promise<{ id: string; linkPagamento: string; qrCode: string; qrCodeBase64: string }> {
    if (this.mockMode) {
      const mock = {
        id: `mock-payment-${Date.now()}`,
        linkPagamento: 'https://sandbox.asaas.com/mock-link',
        qrCode: '00020126580014BR.GOV.BCB.PIX0136mock-key520400005303986540510.005802BR5913OBRA10MOCK6009SAO PAULO62070503***6304ABCD',
      ...
      this.logger.log(`[MOCK ASAAS] gerarCobrancaPix → ${JSON.stringify(mock)}`);
      return mock;
    }
  ...
  async cobrarCartaoRecorrente(dto: {
    ...
    if (this.mockMode) {
      ...
      this.logger.log(`[MOCK ASAAS] cobrarCartaoRecorrente → ${JSON.stringify(mock)}`);
      return mock;
    }
    const { data } = await axios.post(`${this.baseUrl}/payments`, {
      customer: dto.idAsaasCliente,
  ...
  async buscarStatusPagamento(idAsaas: string): Promise<string> {
    ...
    const { data } = await axios.get(`${this.baseUrl}/payments/${idAsaas}`, { headers: this.headers });
    return data.status;
  }
}
