...
export class AnexosService {
  ...
  async criarPresignedUpload(obraId: string, criadorId: string, dto: any) {
    const { attachableId, origemAnexo, tipoArquivo, nomeOriginal, mimeType, tamanhoBytes } = dto;
    ...
    if (origemAnexo === 'RDO' || origemAnexo === 'RDO_ATIVIDADE') {
      const rdoBase = await this.prisma.rdo.findFirst({ where: { obraId, deletedAt: null } });
      if (!rdoBase) throw new ForbiddenException('A entidade destino não pertence a esta Obra ativa.');
    }
    ...
    if (process.env.AWS_ACCESS_KEY_ID) {
      const command = new PutObjectCommand({ Bucket: this.bucketName, Key: filePathS3, ContentType: mimeType });
      uploadUrl = await getSignedUrl(this.s3Client, command, { expiresIn: 3600 }); // Upload vale por 1h
    } else {
      this.logger.warn('Gerando Presigned PUT URL FAKE pois AWS_ACCESS_KEY_ID inexiste em Env');
    }
  ...
  async gerarViewerUrlSegura(anexoId: string, obraId: string) {
    ...
    if (!anexo) throw new NotFoundException('Anexo não encontrado, apagado ou não pertence à Obra logada.');

    let viewUrl = `http://localhost:3000/mock-s3-viewdir/${encodeURIComponent(anexo.urlS3)}`;
    
    if (process.env.AWS_ACCESS_KEY_ID) {
      const command = new GetObjectCommand({ Bucket: this.bucketName, Key: anexo.urlS3 });
  ...
  async listarDaOrigem(obraId: string, origemAnexo: any, attachableId: string) {
    return this.prisma.anexo.findMany({
      ...
      select: {
          id: true, tipoArquivo: true, nomeOriginal: true, createdAt: true, criador: { select: { nome: true } }
      }
    });
  }
}
