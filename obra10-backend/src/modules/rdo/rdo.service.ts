...
export class RdoService {
  ...
  private async getRdoBlockChecked(rdoId: string, obraId: string) {
    const rdo = await this.prisma.rdo.findFirst({ where: { id: rdoId, obraId, deletedAt: null } });
    if (!rdo) throw new NotFoundException('RDO não encontrado.');
    if (rdo.status === RdoStatus.APROVADO || rdo.status === RdoStatus.REJEITADO) {
      throw new ForbiddenException(`RDO imutável — status atual: ${rdo.status}. Apenas RDOs em RASCUNHO ou EM_PREENCHIMENTO podem ser editados.`);
    }
    if (rdo.status === RdoStatus.SUBMETIDO) {
      throw new ForbiddenException('RDO submetido aguarda aprovação. Solicite rejeição para editar novamente.');
    }
    return rdo;
  }
  ...
  async create(obraId: string, criadorId: string, dataParams: any) {
    ...
    if (!rawDate) throw new BadRequestException('Campo dataReferencia é obrigatório.');
    const dataRef = new Date(rawDate);
    if (isNaN(dataRef.getTime())) throw new BadRequestException('dataReferencia inválida — use formato YYYY-MM-DD.');
    
    const existe = await this.prisma.rdo.findFirst({
      where: { obraId, dataReferencia: dataRef, deletedAt: null },
    });
    if (existe) throw new BadRequestException('Já existe um RDO aberto para esta data nesta obra.');

    const dadosExtras = dataParams.dadosExtras ?? null;
    if (dadosExtras && !validarDadosExtras(dadosExtras)) {
      throw new BadRequestException('dadosExtras inválido: campos obrigatórios (versao, data) ausentes.');
    }
    ...
    return this.prisma.rdo.create({
      data: {
        ...
        dadosExtras: dadosExtras ? { ...dadosExtras, versao: DADOS_EXTRAS_VERSAO } : undefined,
      },
    });
  }
  ...
  async saveRascunho(rdoId: string, obraId: string, payload: any) {
    ...
    if (!dadosExtras) throw new BadRequestException('Campo dadosExtras é obrigatório para salvar o rascunho.');
  ...
  async addAtividade(rdoId: string, obraId: string, criadorId: string, dto: any) {
    await this.getRdoBlockChecked(rdoId, obraId);
    return this.prisma.rdoAtividade.create({
      data: { rdoId, criadorId, descricao: dto.descricao, frenteServico: dto.frenteServico },
    });
  }
  ...
  async addEfetivo(rdoId: string, obraId: string, criadorId: string, dto: any) {
    ...
    return this.prisma.rdoEfetivo.create({
      data: { rdoId, criadorId, empresaTerceira: dto.empresaTerceira, funcaoCargo: dto.funcaoCargo, quantidade: Number(dto.quantidade) },
    });
  }

  async addOcorrencia(rdoId: string, obraId: string, criadorId: string, dto: any) {
    await this.getRdoBlockChecked(rdoId, obraId);
    return this.prisma.rdoOcorrencia.create({
      data: { rdoId, criadorId, tipoOcorrencia: dto.tipoOcorrencia, descricao: dto.descricao, horasPerdidas: dto.horasPerdidas ? Number(dto.horasPerdidas) : null },
    });
  }

  // ===================== TAREFAS COM MOTIVO DE NÃO EXECUÇÃO =====================
  async addTarefa(rdoId: string, obraId: string, criadoPorId: string, dto: any) {
    await this.getRdoBlockChecked(rdoId, obraId);
    return this.prisma.tarefaRdo.create({
      data: {
        ...
        horasExecutadas: dto.horasExecutadas ? Number(dto.horasExecutadas) : null,
      },
    });
  }

  async updateTarefa(rdoId: string, tarefaId: string, obraId: string, dto: any) {
    await this.getRdoBlockChecked(rdoId, obraId);
    const tarefa = await this.prisma.tarefaRdo.findFirst({ where: { id: tarefaId, rdoId } });
    if (!tarefa) throw new NotFoundException('Tarefa não encontrada neste RDO.');
    ...
    return this.prisma.tarefaRdo.update({
      ...
      data: {
        ...
        horasExecutadas: dto.horasExecutadas !== undefined ? Number(dto.horasExecutadas) : tarefa.horasExecutadas,
      },
    });
  }
  ...
  async getRecentProfissionais(obraId: string, usuarioId: string) {
    ...
    return recentes
      .filter(r => { const k = `${r.empresaTerceira}:${r.funcaoCargo}`; if (seen.has(k)) return false; seen.add(k); return true; })
      .slice(0, 10);
  }
  ...
  private validateTarefasMotivoAntesDe(tarefas: any[]) {
    const pendentes = tarefas.filter(
      t => (t.statusExecucao === StatusExecucaoTarefa.NAO_EXECUTADO || t.statusExecucao === StatusExecucaoTarefa.PARCIAL) && !t.motivoNaoExecucao,
    );
   ...
   */
  async submeter(id: string, obraId: string, userObraRole: any, aprovadorIdSelecionado?: string) {
    const rdo = await this.prisma.rdo.findFirst({
      where: { id, obraId, deletedAt: null },
      include: { tarefas: true, criador: { select: { id: true, nome: true, email: true } }, obra: { select: { nome: true } } },
    });
    if (!rdo) throw new NotFoundException('RDO não encontrado.');
    if (rdo.status !== RdoStatus.RASCUNHO && rdo.status !== RdoStatus.EM_PREENCHIMENTO) {
      throw new BadRequestException(`Não é possível submeter RDO no status: ${rdo.status}`);
    }
    ...
    if (aprovadorInfo) {
      ...
      this.email.sendAprovacaoRdoPendente(
        aprovadorInfo.email,
        aprovadorInfo.nome,
        rdo.criador.nome,
        rdo.obra.nome,
        ...
        obraId,
        id,
      ).catch(err => console.error('[RdoService] Falha ao enviar e-mail de aprovação pendente:', err));
    }

    return rdoAtualizado;
  }

  async aprovar(id: string, obraId: string, userObraRole: any, aprovadorId: string) {
    if (userObraRole.perfilId < 3) {
      throw new ForbiddenException('Apenas gestores/engenheiros (perfil 3+) podem APROVAR um RDO.');
    }
    ...
    if (!rdo || rdo.obraId !== obraId) throw new NotFoundException('RDO inválido.');
    if (rdo.status !== RdoStatus.SUBMETIDO) {
      throw new BadRequestException(`RDO precisa estar SUBMETIDO para aprovação. Status atual: ${rdo.status}`);
    }

    const aprovador = await this.prisma.usuario.findUnique({ where: { id: aprovadorId }, select: { nome: true } });
    return this.prisma.rdo.update({
      where: { id },
      data: {
  ...
  async rejeitar(id: string, obraId: string, userObraRole: any, aprovadorId: string, motivo: string) {
    if (userObraRole.perfilId < 3) {
      throw new ForbiddenException('Apenas gestores/engenheiros podem REJEITAR um RDO.');
    }
    ...
    if (!rdo || rdo.obraId !== obraId) throw new NotFoundException('RDO inválido.');
    if (rdo.status !== RdoStatus.SUBMETIDO) {
      throw new BadRequestException(`RDO precisa estar SUBMETIDO para ser rejeitado. Status atual: ${rdo.status}`);
    }
  ...
  async revisar(id: string, obraId: string) {
    const rdo = await this.prisma.rdo.findFirst({ where: { id, obraId, deletedAt: null } });
    if (!rdo) throw new NotFoundException('RDO não encontrado.');
    if (rdo.status !== RdoStatus.REJEITADO) {
      throw new BadRequestException('Apenas RDOs REJEITADOS podem ser reabertos para revisão.');
    }
    return this.prisma.rdo.update({
      where: { id },
  ...
  async getStats(obraId: string, dataInicio?: string, dataFim?: string) {
    const inicio = dataInicio ? new Date(dataInicio) : (() => { const d = new Date(); d.setDate(d.getDate() - 30); return d; })();
    const fim = dataFim ? new Date(dataFim) : new Date();

    const rdos = await this.prisma.rdo.findMany({
      where: { obraId, deletedAt: null, dataReferencia: { gte: inicio, lte: fim } },
    ...
    const rdosAprovados = rdos.filter(r => r.status === RdoStatus.APROVADO).length;
    const todasTarefas = rdos.flatMap(r => r.tarefas);
    const totalTarefas = todasTarefas.length;
    const tarefasExecutadas = todasTarefas.filter(t => t.statusExecucao === StatusExecucaoTarefa.EXECUTADO).length;

    const motivoMap: Record<string, number> = {};
    todasTarefas
      .filter(t => t.motivoNaoExecucao)
      .forEach(t => { const m = String(t.motivoNaoExecucao); motivoMap[m] = (motivoMap[m] ?? 0) + 1; });
    const motivosNaoExecucao = Object.entries(motivoMap)
      .map(([motivo, total]) => ({ motivo, total }))
      .sort((a, b) => b.total - a.total);

    const funcaoMap: Record<string, number> = {};
    rdos.flatMap(r => r.efetivos).forEach(e => {
      funcaoMap[e.funcaoCargo] = (funcaoMap[e.funcaoCargo] ?? 0) + e.quantidade;
    });
    ...
    const semanaMap: Record<string, { executadas: number; naoExecutadas: number }> = {};
    rdos.forEach(r => {
      const semana = this.getSemanaLabel(r.dataReferencia);
      if (!semanaMap[semana]) semanaMap[semana] = { executadas: 0, naoExecutadas: 0 };
      r.tarefas.forEach(t => {
        if (t.statusExecucao === StatusExecucaoTarefa.EXECUTADO) semanaMap[semana].executadas += 1;
        else semanaMap[semana].naoExecutadas += 1;
      });
    });
    const execucaoPorSemana = Object.entries(semanaMap)
      .map(([semana, v]) => ({ semana, ...v }));

    return { totalRdos, rdosAprovados, totalTarefas, tarefasExecutadas, motivosNaoExecucao, horasPorFuncao, execucaoPorSemana };
  }
