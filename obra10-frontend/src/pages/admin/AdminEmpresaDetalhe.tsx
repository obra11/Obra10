...
export const AdminEmpresaDetalhe: React.FC = () => {
  ...
  const handleConfirmarEmailAdmin = async () => {
    setReenviandoVerificacao(true);
    try {
      await api.patch(`/admin/empresas/${id}`, { emailVerificado: true });
      await fetchEmpresa();
      alert('E-mail confirmado. A empresa já pode gerar PIX.');
    } catch (e: any) {
      alert(e?.response?.data?.message || 'Erro ao confirmar e-mail.');
    } finally {
      setReenviandoVerificacao(false);
    }
  };
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 flex flex-col md:flex-row gap-6 justify-between items-start mb-6 shadow-sm">
        ...
        <div className="flex gap-3 flex-wrap">
          {!empresa.emailVerificado && (
            <>
              <button
                onClick={handleConfirmarEmailAdmin}
                disabled={reenviandoVerificacao}
                className="flex items-center gap-2 px-4 py-2 border border-green-200 text-green-800 bg-green-50 hover:bg-green-100 rounded-lg text-sm font-semibold transition-colors disabled:opacity-60"
              >
