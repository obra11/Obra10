...
/** Mesmos rótulos do cadastro de usuários (gestor). */
...
function labelTipoCadastro(perfilGlobal: string) {
  return TIPO_CADASTRO_LABELS[perfilGlobal] || perfilGlobal;
}

function tipoCadastroBadgeClass(perfilGlobal: string) {
  switch (perfilGlobal) {
    case 'GESTOR':
    case 'SUPER_ADMIN':
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 md:mb-8 gap-3">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            Gerencie quem tem acesso a esta obra e suas permissões.
          </p>
        </div>
        {canManage && (
          <button
              ...
              <table className="w-full text-left">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-sm font-semibold text-gray-600">Tipo no cadastro</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {colaboradores.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${tipoCadastroBadgeClass(c.usuario.perfilGlobal)}`}
                        >
                          {labelTipoCadastro(c.usuario.perfilGlobal)}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                            ...
                            <>
                              {Object.keys(c.permissoes || {}).map((mod) => {
                                ...
                                return (
                                  <span
                                    key={mod}
                                    className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-semibold border shadow-sm ${getPermColor(val)}`}
                                  >
                                    <Shield size={12} className="mr-1" />
                      ...
                      {canManage && (
                        <td className="px-6 py-4 text-right space-x-2">
                          ...
                          <button
                            onClick={() => handleExcluir(c.usuario.id)}
            ...
            {colaboradores.map((c) => (
              <div key={c.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium shrink-0 ml-2 ${tipoCadastroBadgeClass(c.usuario.perfilGlobal)}`}
                  >
                    {labelTipoCadastro(c.usuario.perfilGlobal)}
                  </span>
                </div>
                    ...
                    <>
                      {Object.keys(c.permissoes || {}).map((mod) => {
                        ...
                        return (
                          <span
                            key={mod}
                            className={`inline-flex items-center px-2 py-1 rounded-md text-[10px] font-semibold border ${getPermColor(val)}`}
                          >
                ...
                {canManage && (
                  <div className="flex gap-2 pt-3 border-t border-gray-100">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg active:bg-blue-100"
                    >
                      <Edit2 size={14} /> Editar
                    </button>
                    <button
                      onClick={() => handleExcluir(c.usuario.id)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 text-sm font-medium text-red-600 bg-red-50 rounded-lg active:bg-red-100"
                    >
